...
Merge TransDecoder .cds files before running extract_cds.sh script
```{console}
...
```{console}
$ # for the aggregated assemblies, cut at first underscore
$ or i in ../nogaps/*fa; do name=$(basename $i); sed 's/_.*//' ${i} > ./${name}; done 

$ # for the individual assemblies, cut at second underscore to retain number ID
...
Find overlapping genes in both the SCO expression analyses and the SCO phylogenetic tree analyses
```{console}
$ # use grep to search content of one file in another
$ grep -Ff taxonA-taxonB_group_expression.txt taxonA-taxonB_group_tree.txt

```

For each individual rep contig assembly get a list of filtered SCOs for SNP calling
Need to alter sample name each (12) time 
```{console}
$ # get the list of TransDecoder IDs
$ for i in /path/to/nogaps/*.fa; do grep taxon_n ${i} >> ./headers_taxon_n.txt; done

$ # # remove the ".p" extension
$ sed 's/\..*//' headers_taxon_n.txt > endtrim_taxon_n.txt

$ # remove taxon and sample ID (leave only Trinity isoform ID)
$ sed 's/>taxon_x_//' endtrim_taxon_n.txt > clean_taxon_n.text
```
